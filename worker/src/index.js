...
import { getScorecard, getModels } from './store.js';
  ...
  async fetch(req, env) {
    ...
    try {
      ...
      if (url.pathname === '/api/health') {
        return json({ ok: true, tracked: TRACKED, ts: Date.now() });
      }
    ...
    return json({ error: 'not found', endpoints: ['/api/scorecard', '/api/model', '/api/health'] }, 404);
  },
};
