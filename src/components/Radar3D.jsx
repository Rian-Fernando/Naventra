...
// Top-down airliner silhouette (nose +Z after the rotation below).
function buildPlaneGeo() {
  const s = new THREE.Shape();
  s.moveTo(0, 1.0);
  s.lineTo(0.16, 0.62); s.lineTo(0.16, 0.18);
  s.lineTo(0.95, -0.22); s.lineTo(0.95, -0.42); s.lineTo(0.16, -0.22);
  s.lineTo(0.14, -0.62);
  s.lineTo(0.42, -0.85); s.lineTo(0.42, -1.0); s.lineTo(0, -0.88);
  s.lineTo(-0.42, -1.0); s.lineTo(-0.42, -0.85); s.lineTo(-0.14, -0.62);
  s.lineTo(-0.16, -0.22); s.lineTo(-0.95, -0.42); s.lineTo(-0.95, -0.22);
  s.lineTo(-0.16, 0.18); s.lineTo(-0.16, 0.62);
  s.closePath();
...
// Rotorcraft: cabin + tail boom + rotor disc, merged into one geometry.
function buildHeliGeo() {
  const body = new THREE.SphereGeometry(0.55, 8, 6);
  body.scale(1, 0.55, 1.5);
  const boom = new THREE.BoxGeometry(0.16, 0.12, 1.5);
  boom.translate(0, 0, -1.2);
  const rotor = new THREE.CylinderGeometry(1.5, 1.5, 0.05, 10);
  rotor.translate(0, 0.42, 0);
  const geo = mergeGeometries([body, boom, rotor]);
...
export default function Radar3D({ airport, aircraft, conflicts, runways, selectedId, onSelect, range, labels, showTrails, onUnavailable }) {
  ...
  useEffect(() => {
    ...
    const planeGeo = buildPlaneGeo();
    const heliGeo = buildHeliGeo();
    ...
    function getObj(id, isHeli) {
      let o = pool.get(id);
      if (o) return o;
      const group = new THREE.Group();
      const cone = new THREE.Mesh(isHeli ? heliGeo : planeGeo, mats.ENROUTE);
      ...
      o = { group, cone, stem, base, selRing, hit, trailLine };
      pool.set(id, o);
      return o;
    }
    ...
    const animate = () => {
      ...
      for (const ac of acs) {
        ...
        const o = getObj(ac.id, ac.category === 'A7');
        ...
        o.cone.scale.setScalar(ac.onGround ? 0.7 : 1);
